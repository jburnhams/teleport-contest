...
- The `P_PONY` riding skill edge case has been resolved and is actively checking `PM_PONY` property correctly without omitting C-code parity. Appeased logic rules restoring the `P_RIDING` implementation parity utilizing the specific property `gu.urole.petnum == PM_PONY` verified against the latest `weapon.c`.
...
- Extracted `roles`, `races`, `aligns`, and `genders` into `js/roles.js`, extracting and formatting `roles`, `races`, `aligns`, and `genders` tables from C source.
- Corrected object class constants (`WEAPON_CLASS`, etc.) in `js/const.js`. Confirmed missing object class constants and added/corrected them.
...
### Stream E: Monster System (continued)
...
### Stream D: Object System (continued)
