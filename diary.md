...
## ${today}
- Ported `makemon`, `goodpos`, `enexto`, `collect_coords`, `m_initgrp`, `peace_minded`, `set_malign`, and `set_mimic_sym` to `js/mkmon.js`.
- Added `newmonhp` and necessary constants to `js/const.js`.
- Next up: integrate makemon into mklev to verify exact PRNG footprint and remove stubs.
