...
- Completed Stream F2 (`ini_inv`) stubbing the correct RNG sequence according to C `u_init.c` and `mkobj.c`. Replicated quantity adjustment and BUC evaluation correctly using `while (quan > 0)`. Fixed `got_sp1` flag tracking to correctly update state inside `ini_inv`. Replaced fragile hardcoded item class checking with `objects` metadata mapping exactly as directed.
- Completed Stream F5 by porting `newexplevel` (as `check_experience`), `pluslvl`, `losexp`, and `rndexp` to `exper.js` to handle the experience level RNG and logic. Refactored `enermod` correctly using the proper `PM_` constants from `const.js`.
...
- Successfully passed the regression check preserving 88/11406 score! Next: Move onto `goodpos` and `enexto` functionality!
