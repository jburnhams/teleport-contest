...
- Corrected and confirmed missing object class constants (`WEAPON_CLASS`, etc.) in `js/const.js`.
- Verified `test/roles.test.js` passes and `npm run score:check` maintains score parity without regressions.
- What's next: Begin work on Stream D (Object System) or Stream E (Monster System) since Stream A dependencies are met. Investigating `mksobj_init` logic (~200 lines).

### Baseline Fuzzer Run
- **Condition**: `--moves 0` (chargen + mklev only)
- **Seeds Tested**: 20 seeds (100-119)
- **Pass Rate**: 0/20 (0%) (Previously 100% (20/20) for chargen+mklev)
- **Top Divergences** (Most frequent first-divergence locations across 44 canonical sessions):
  - `makelevel(mklev.c:1410)` (8 times)
  - `lspo_map(sp_lev.c:6163)` (6 times)
  - `fill_special_room(sp_lev.c:2769)` (6 times)
  - `mkobj(mkobj.c:281)` (3 times)
  - `somex(mkroom.c:669)` (2 times)
  - `mkclass_aligned(makemon.c:1946)` (2 times)
  - `makelevel(mklev.c:1295)` (2 times)
  - `traptype_rnd(mklev.c:1951)` (1 time)
  - `nh.rn2 src=themerms.lua:1039 parent=room([C]:-1)` (1 time)
  - `newpw(exper.c:52)` (1 time)
  - `fill_ordinary_room(mklev.c:998)` (1 time)
  - `blessorcurse(mkobj.c:1848)` (1 time)
