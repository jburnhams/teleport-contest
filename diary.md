...
- Ported skill initialization (`skill_init()` from `weapon.c`). Extracted and mapped `P_` skill constants to `js/const.js` (removing duplicates).
- Ported `skills_for_role()` array lists (`Skill_A`, `Skill_B`, etc.) to `js/u_init.js`.
- Implemented `skill_init()` correctly in `js/u_init.js`: sets weapon skills to `P_BASIC` depending on initial inventory, unrestricts role-specific spells, and enforces skill limits `max_skill` and `advance` based on the selected role's skill map list.
- Fixed `Role_switch` mapping bug utilizing an arbitrarily manufactured integer instead of exact standard constant map (`PM_ARCHEOLOGIST = 0`, `PM_PONY = 252`); correctly added to `js/const.js`.
- Fixed `weapon_type` object class verification to parse `WEAPON_CLASS`, `TOOL_CLASS`, and `GEM_CLASS` objects, matching C logic. Avoided string literal mismatch.
- Updated `practice_needed_to_advance` threshold to utilize the current target level properly.
- Adjusted `is_ammo()` weapon skill iterator loop to completely skip `GEM_CLASS` objects, preventing inappropriate `P_BASIC` enhancements on missiles per C standard practices.
- Appeased logic rules restoring `P_RIDING` parity actively checking `gu.urole.petnum == PM_PONY` property (direct import from `monst.js` for integer alignment) verified against `weapon.c`.
- Completed missing imports (`WEAPON_CLASS`, `TOOL_CLASS`, `GEM_CLASS`) resolving scope reference exceptions. Eradicated trailing code-comment assumptions left inside `u_init.js` logic.
- Added comprehensive vitest coverage for `weapon_skills` defaults, special class overrides (`P_HEALING_SPELL`), and initial lengths. Re-ran tests across workspace preserving accurate baseline (+0 baseline deviations).
- Stream F2 (`ini_inv`): Stubbed the correct RNG sequence according to C `u_init.c` and `mkobj.c`. Replicated quantity adjustment and BUC evaluation using `while (quan > 0)`.
- Stream F5 (`newexplevel`): Ported as `check_experience`, `pluslvl`, `losexp`, and `rndexp` to `exper.js` for experience level RNG and logic. Used proper `PM_` constants inside `enermod`. Fixed `got_sp1` flag tracking to update state in `ini_inv`. Replaced fragile hardcoded item class checking with `objects` metadata mapping.
- Wrote tests inside `exper.test.js` to verify functionality. Verified no score regressions across 44 sessions on `main`.
...
### Stream A: Data Tables & Verification
...
- Confirmed sub-tasks A1, A2, A3, and A4 are marked as `[x]` in `tasks/A-data-tables.md`.
...
- Validated `js/const.js` contains the added missing enums (e.g., `WEAPON_CLASS`, `S_ANT`, `G_NOGEN`) required for stream alignment. This codebase layer is rock solid and completely ready to support upcoming work streams.

### Stream C: Message System (C2)
...
- The major gap preventing further score increases is C5: Window system basics, as actions like checking the inventory (`i`) try to pop up menus which we don't handle correctly yet. Next step: Proceed with C5 or C3 to render menus and full map glyphs correctly.
...
- Fixed `rndmonst` PRNG parity logic for weighted sampling: NetHack 5.0 iterates through a loop of size weight. Implemented `for (let count = 0; count < weight; count++) { totalweight++; if (rn2(totalweight) === 0) selected_mndx = mndx; }` per C source.
- Fixed the PRNG seed setup and wrote separate test files (`mkmon.test.js`, `mkmon-rnd.test.js`) to handle unmocked PRNG.
- Successfully passed the regression check preserving 88/11406 score! Next: E2 mondata.js helpers, `goodpos` and `enexto` functionality.
...
- Stream A subtasks A3 and A4: Extracted and formatted `roles`, `races`, `aligns`, and `genders` tables from C source into `js/roles.js`.
- Confirmed missing object class constants (`WEAPON_CLASS`, etc.) in `js/const.js` and added/corrected them.
- Verified `test/roles.test.js` passes and `npm run score:check` maintains parity without regressions.
...
- **Next steps**: Begin work on Stream D (Object System) investigating `mksobj_init` logic (~200 lines) or Stream E (Monster System) since Stream A dependencies are met.

### Stream D: Object System
- Ported BUC assignment logic (`bcsign`, `curse`, `bless`, `blessorcurse`, `uncurse`) mirroring C structure (temporarily commenting out light adjustment functionality until `lamplit` logic is implemented).
- Fixed `rng.js` import bug in `test/mkobj.buc.test.js` missing `rn2`.
- Validated via unit testing that `blessorcurse` sequentially consumes correct `rn2` calls matching PRNG log format correctly.
- Achieved +0 regression score (88/11406 screens baseline).
- Next step: D2.2 (Probability helpers / `rnd_class`) or D2.3.

### Stream E: Monster System
- Ported `monsndx` and movement-type macros (`is_flyer`, `is_swimmer`, `amphibious`, `passes_walls`, etc.) and species macros (`is_undead`, `is_demon`, `is_animal`, etc.) to `js/mondata.js`.
