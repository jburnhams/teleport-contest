...
**Execution Statistics**: 123 steps, 35386 calls to `rn2()` and its brethren.


This session serves as a high-entropy mechanical validation of the engine's endgame infrastructure. The user entity (a solitary, wandering '@')—a Barbarian named *Wizard*—utilises administrative powers to jump directly into the Elemental Planes, forcing the engine to resolve complex environmental flags like extreme heat and lack of gravity.

### Mechanical Sequence

**Steps 1–3: The Plunderer Setup**
The engine performs 2,510 calls to `rn2()` and its brethren to generate the initial state (St:18/02 Dx:17 Co:17 In:8 Wi:7). Wizard begins on Dungeon Level 1 and declines the tutorial.
...
- 54 calls to `rn2()` and its brethren resolve the immediate environmental triggers.
...
- Step 110: A massive **2,907 calls to `rn2()` and its brethren** resolve the generation of the Plane of Air.
