...
**Execution Statistics**: 86 steps, 3026 calls to `rn2()` and its brethren.


This session serves as a high-precision mechanical validation of the engine's object-generation and monster-transformation subroutines. The user entity (a solitary, wandering '@')—a Wizard appropriately named *Wizard*—demonstrates the engine's ability to create specific items and entities via administrative commands and then resolves a complex state-change interaction between them.

### Mechanical Sequence

**Steps 1–39: The Wish for Change**
The engine performs 2,764 calls to `rn2()` and its brethren to generate the initial state (St:10 Dx:14 Co:15 In:18 Wi:7). Wizard immediately invokes the administrative wishing command `#wizwish`.
...
- At Step 72, the engine resolves the spawn logic. 51 calls to `rn2()` and its brethren are consumed as the new monster entity materialises in an adjacent tile: *A jackal appears next to you.*
...
- 14 calls to `rn2()` and its brethren resolve the beam trajectory and the initial contact.
- While the final transformation is not explicitly messaged in the short log, the consumption of calls to `rn2()` and its brethren confirms the activation of the polymorph logic.
