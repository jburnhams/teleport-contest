...
**Execution Statistics**: 35 steps, 3656 calls to `rn2()` and its brethren.


This session serves as a rigorous mechanical validation of the engine's status effect subroutines. The user entity (a solitary, wandering '@')—a Gnomish Healer named *Healix*—demonstrates the engine's ability to handle nutrition, the "New Moon" luck penalty, and the precise deterministic duration of self-inflicted sleep.

### Mechanical Sequence

**Steps 1–2: The New Moon**
The engine performs 2,491 calls to `rn2()` and its brethren to generate the universe and character attributes (St:8 Dx:10 Co:14 In:12 Wi:14 Ch:17). The initialisation logic detects the local system clock and applies a deterministic luck penalty. The message line warns: *Be careful! New moon tonight.*

**Steps 6–7: The Macintosh Validation**
Healix decides to address his starting hunger. He issues the eat command (`e`) and selects an apple (`j`). The engine calculates the nutritional value and any potential effects, outputting a highly specific aesthetic message: *Delicious! Must be a Macintosh!* 7 calls to `rn2()` and its brethren are consumed as his nutrition level is updated.
...
- A massive 963 calls to `rn2()` and its brethren are consumed as the engine resolves the hit and calculates the random duration of the resulting sleep status.
...
At Step 23, the sleep counter reaches zero. 67 calls to `rn2()` and its brethren are consumed as the engine resolves the final monster movements and the wake-up event: *You wake up.*

**Steps 24–35: Post-Slumber Analytics**
Healix immediately checks his inventory linked list, known spells, and discoveries to ensure his mental state is intact. He confirms his Constitution (14) and Intelligence (12) before performing two final searches (`s`) and inspecting the floor (`:`), presumably looking for the gold his kitten was busy collecting while he was napping.
