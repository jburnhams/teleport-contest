...
Unlike standard runs which rely on automatic generation, the user asserts direct control over the character creation process. They type `S, e, p, t, o, r` to name the entity. At Step 7, they explicitly decline automatic role selection (`n`), instead navigating the menus to manually request the role of Rogue (St:13 Dx:17 Co:14 In:10 Wi:10 Ch:11) (`r`), the male gender (`m`), and the Orcish race (`o`).
