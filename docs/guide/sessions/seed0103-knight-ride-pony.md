...
**Execution Statistics**: 59 steps, 2640 calls to `rn2()` and its brethren.


This brief but poignant session serves as a rigorous mechanical validation of the engine's riding subroutines. The user entity (a solitary, wandering '@')—a human Knight appropriately named *Sir*—demonstrates the catastrophic deterministic consequences of attempting to mount a pony with low Dexterity (8).

### Mechanical Sequence

**Steps 1–4: The Gallant Sir**
The engine performs 2,432 calls to `rn2()` and its brethren to generate the universe and Sir's attributes (St:15 Dx:8 Co:11 In:8 Wi:14 Ch:17). Sir begins on Dungeon Level 1, accompanied by a saddled pony (represented with stunning realism as a 'u'), and declines the tutorial.
...
- 2 calls to `rn2()` and its brethren resolve the check.
- The roll is a failure. The engine calculates the fall damage.
- The message line declares: *You slip while trying to get on the saddled pony (represented with stunning realism as a 'u').*
...
- Step 57: Another failure. 2 calls to `rn2()` and its brethren resolve the fall.
- The message line repeats: *You slip while trying to get on the saddled pony (represented with stunning realism as a 'u').*
...
It is a succinct mechanical validation of the riding interaction matrix, proving that the engine can successfully calculate lethal fall damage from a stationary animal. Sir's grand adventure lasted exactly 58 steps, ended not by a terrifying dragon (which, mathematically speaking, is just a capital 'D') or a demon, but by the relentless mathematics of a saddle he was too clumsy to reach.
