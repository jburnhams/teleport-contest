...
**Execution Statistics**: 713 steps, 59178 calls to `rn2()` and its brethren.


This session provides a rigorous mechanical shakedown of the object dipping subroutines. The user entity (a solitary, wandering '@')—a Lawful Dwarven Valkyrie named *Dequa*—demonstrates the engine's ability to handle complex item-environment interactions and the resulting legal repercussions in a procedurally generated dungeon.

### Mechanical Sequence

**Steps 1–13: The Stripling Valkyrie**
Dequa is created through a sequence of manual menu selections (`v` for Valkyrie, `d` for Dwarf). The engine calculates her impressive Strength (18/02) and Constitution (18) across 2,953 calls to `rn2()` and its brethren. Upon materialising, she is greeted by the light of a Full Moon, granting her a deterministic luck bonus.
...
Dequa encounters a fountain (a bubbling '{' of procedural doom) and initiates the `#dip` extended command. She selects her orcish helm (`h`) and confirms her desire to submerge it. The engine calculates the probability of various outcomes. In this first instance, 97 calls to `rn2()` and its brethren are consumed, but the fountain remains largely unresponsive to the presence of orcish headgear.
...
It is a comprehensive validation of the fountain interaction matrix, proving that the engine can successfully manage the transition from "cleaning an orcish helm (a glorious '[' offering dubious protection)" to "accidental public bathing and subsequent police harassment" without missing a single PRNG call.
