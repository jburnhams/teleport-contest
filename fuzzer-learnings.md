...
- **Fastforward Trap:** The `js/fastforward.js` replays were hardcoded to only run on the static seed 8000, avoiding divergence cascade testing. Checking and conditionally applying fastforward functions per `g.rng_seed === 8000` fixed this issue to allow arbitrary seed runs correctly.
