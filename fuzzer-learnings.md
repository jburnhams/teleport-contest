...
9. **RNG Normalisation**: Regular expression handling in `fuzz-diff.mjs` and `ps_test_runner.mjs` requires careful handling of types. Applying `.replace` to non-strings can lead to silent errors or crashes. Converting inputs to strings before replacement resolves this issue.
