...
**Your job this session:** Make small, steady progress on **Stream F** from `tasks/F-hero-init.md`. Work one sub-task at a time, verifying after each before moving on.
...
1. `tasks/F-hero-init.md` — your task checklist for this stream
...
For each checklist item in `tasks/F-hero-init.md`:
...
5. **Tick the checklist** — edit `tasks/F-hero-init.md` and mark the item `[x]`.
6. **Commit** — one commit per sub-task, message format: feat(stream-F): <what was done>
...
Read `tasks/F-hero-init.md` now and identify:
