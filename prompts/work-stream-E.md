...
**Your job this session:** Make small, steady progress on **Stream E** from `tasks/E-monster-system.md`. Work one sub-task at a time, verifying after each before moving on.
...
1. `tasks/E-monster-system.md` — your task checklist for this stream
...
For each checklist item in `tasks/E-monster-system.md`:
...
5. **Tick the checklist** — edit `tasks/E-monster-system.md` and mark the item `[x]`.
6. **Commit** — one commit per sub-task, message format: feat(stream-E): <what was done>
...
Read `tasks/E-monster-system.md` now and identify:
