...
**Your job this session:** Make small, steady progress on **Stream D** from `tasks/D-object-system.md`. Work one sub-task at a time, verifying after each before moving on.
...
1. `tasks/D-object-system.md` — your task checklist for this stream
...
For each checklist item in `tasks/D-object-system.md`:
...
5. **Tick the checklist** — edit `tasks/D-object-system.md` and mark the item `[x]`.
6. **Commit** — one commit per sub-task, message format: feat(stream-D): <what was done>
...
Read `tasks/D-object-system.md` now and identify:
