...
**Your job this session:** Make small, steady progress on **Stream B** from `tasks/B-rng-foundation.md`. Work one sub-task at a time, verifying after each before moving on.
...
1. `tasks/B-rng-foundation.md` — your task checklist for this stream
...
For each checklist item in `tasks/B-rng-foundation.md`:
...
5. **Tick the checklist** — edit `tasks/B-rng-foundation.md` and mark the item `[x]`.
6. **Commit** — one commit per sub-task, message format: feat(stream-B): <what was done>
...
Read `tasks/B-rng-foundation.md` now and identify:
