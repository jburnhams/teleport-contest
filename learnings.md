...
## mkobj
- \`mksobj_init\` consumes varying amounts of RNG depending on the exact object class and item type. The \`blessorcurse\` logic uses \`rn2(4)\` for scrolls and potions, but varies for weapons and tools. The order of RNG consumption within \`mksobj_init\` is very strict to preserve PRNG parity.
- \`rnd_class\` uses \`oclass_prob_totals\` and \`bases\`, both computed by \`init_oclass_probs\` inside \`o_init.js\`.
- We porting \`o_init\` functions that initialized probabilities to maintain rng parity during \`rnd_class\` inside \`mkobj\`.

## u_init
- \`ini_inv\` uses \`ini_inv_mkobj_filter\` when \`trclass\` is present but \`trotyp\` is UNDEF_TYP. This loop creates \`mkobj\` and drops certain types (like \`WAN_WISHING\`, \`RIN_LEVITATION\`, etc.) until it finds an acceptable object, which consumes RNG per attempt.
