...
## Monster Creation & goodpos / enexto
- **collect_coords**: Implements concentric square ring searches and consumes `rn2` for shuffling rings when candidates are found.
- **makemon**: Calls `rndmonst()` in a loop up to 50 times if a specific monster pointer is not provided. Then calls `newmonhp` which consumes `d()` and `rnd()`.
